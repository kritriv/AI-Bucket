...
const RemoveItem = async (req,res)=>{
    try{
        ...
        handleApiResponse(res,200,'listing deleted successful',
            {Deleted: formattedDeletedlisting,
            });
        }catch (error){
            const errorMessage = error.message.includes('Invalid ID format')? 'Use a Proper Id' : `An error occurred while deleting the listing: ${error.message}`;
